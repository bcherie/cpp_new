...
void Form::execute(Bureaucrat const & executor) const
{
	...
	if (this->is_signed == false)
		throw std::logic_error (this->f_name + " is not signed. Can't execute...");
}
...
{
	out << "Status: This form is signed when " << obj.get_gradeSign() << " and " << obj.get_gradeExecute() << " to sign " << obj.getName() << std::endl;
	return (out);
}
